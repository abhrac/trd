...
class GNNAggHausdorff(nn.Module):
    ...
    def train_one_epoch(self, trainloader, epoch, save_path):
        ...
        for i, data in enumerate(tqdm(trainloader)):
            ...
            # views = self.compute_views(im)
            # hdist = hausdorff_distance(views[0], views[1])
            hdist = hausdorff_distance(semrel_graphs, semrel_graphs)

            loss = self.criterion(sem_logits + (self.local_weight * local_logits) + global_logits, labels) + self.local_weight * hdist
